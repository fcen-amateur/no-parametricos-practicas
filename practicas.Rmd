...
df %>%
  ggplot(aes(x = rank(temp), y = temp, color=ciudad)) +
  geom_point()
```
...
p_valor.normal(Tobs, muT, sqrt(varT), "mayor")
# p-valor exacto
1 - pW(Tobs - 1, n, m)
...
```{r p4e4}
df <- df <- read.csv(file.path(DATA_DIR, "p4e4.csv")) %>%
  gather("genero", "temp")
...
N <- n + m
rgs <- rank(c(x, y))
Tobs <- sum(rgs[(n+1):(n+m)])

# Hay empates, asi que calculo la esperanza, varianza y correcion por empates
muT <- n*(N+1)/2
varT <- (m*n/(N*(N+1))*sum(rgs^2)) - (m*n*(N+1)^2)/(4*(N-1))
# p-valor aproximacion normal
p_valor.normal(Tobs, muT, sqrt(varT), "menor")
# p-valor exacto
1 - pW(Tobs - 1, 9, 8)
